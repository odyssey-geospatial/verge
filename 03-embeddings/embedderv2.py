    ...
    def forward(self, embeddings: torch.Tensor, similarity_features: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(batch_size):
            if positive_mask[i].sum() > 0:  # If there are positive pairs for this instance
                ...
                negative_mask = ~positive_mask[i] & ~torch.eye(batch_size, dtype=torch.bool, device=embeddings.device)[i]
                negative_scores = embedding_similarity[i][negative_mask]
        ...
        # If no positive pairs found, return a small regularization loss to keep gradients flowing
        if num_positives == 0:
            # Simple regularization: encourage unit norm embeddings
            norm_loss = torch.mean((torch.norm(embeddings, dim=1) - 1.0) ** 2)
            return norm_loss
